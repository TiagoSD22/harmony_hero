...
import com.guitarvisualizer.service.LLMHarmonyService
import com.guitarvisualizer.handler.HarmonyHandler
...
class HttpRequestHandler(repository: ChordRepository, harmonyService: LLMHarmonyService) extends SimpleChannelInboundHandler[FullHttpRequest] {
  private val logger = LoggerFactory.getLogger(getClass)
  private val harmonyHandler = new HarmonyHandler(harmonyService)
  ...
  override def channelRead0(ctx: ChannelHandlerContext, request: FullHttpRequest): Unit = {
    ...
    val response = (method, uri) match {
      case (HttpMethod.GET, "/health") =>
        handleHealth()
      case (HttpMethod.POST, "/api/harmony") =>
        handleHarmonyRequest(request)
  ...
  private def handleHarmonyRequest(request: FullHttpRequest): (HttpResponseStatus, String) = {
    try {
      val content = request.content().toString(CharsetUtil.UTF_8)
      harmonyHandler.handleHarmonyRequest(content) match {
        case Success(jsonResponse: String) =>
          (HttpResponseStatus.OK, jsonResponse)
        case Failure(ex) =>
          logger.error("Failed to process harmony request", ex)
          createErrorResponse(HttpResponseStatus.INTERNAL_SERVER_ERROR, "Failed to analyze harmony")
      }
    } catch {
      case ex: Exception =>
        logger.error("Error processing harmony request", ex)
        createErrorResponse(HttpResponseStatus.BAD_REQUEST, "Invalid request format")
    }
  }
